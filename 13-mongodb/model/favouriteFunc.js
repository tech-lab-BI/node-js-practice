const fs = require("fs");
const path = require("path");
const { getData } = require("./homesFunc");

const favouriteDataPath = path.join(__dirname, "../", "data", "favourite.json");

function getFavourite(callback) {
  fs.readFile(favouriteDataPath, (err, data) => {
    if (err || data.length === 0) {
      console.log("Favourite list empty");
      callback([]);
      return;
    }
    callback(JSON.parse(data));
  });
}

function putFavourite(data, callback) {
  fs.writeFile(favouriteDataPath, JSON.stringify(data), (err) => {
    if (err) console.log("Error in write file");
    callback();
  });
}

function getFavouriteList(callback) {
  getFavourite((favHomesId) => {
    getData((homes) => {
      // const favHomeList = favHomesId.map(favHome => homes.find(home => home._id == favhome._id));

      // const favHomeList = [];
      // for(const home of homes){
      //   if(favHomesId.some(fav => fav.id == home._id)){//favHomesId.includes(home._id) favHomeId obj array , home._id value so it return always false
      //     favHomeList.push(home);
      //   }
      // }

      const favHomeList = homes.filter((home) =>
        favHomesId.some((favHome) => favhome._id == home._id),
      );

      callback(favHomeList);
    });
  });
}

function deleteByIdFav(homeId, callback) {
  getFavourite((homes) => {
    homes = homes.filter((home) => home._id != homeId);
    putFavourite(homes, () => {
      callback();
    });
  });
}

module.exports = {
  getFavourite,
  putFavourite,
  getFavouriteList,
  deleteByIdFav,
};
